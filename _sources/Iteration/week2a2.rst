...
.. activecode:: assess_ps_02_08
    ...
    ``week_temps_f`` is a string with a list of fahrenheit temperatures separated by the ``,`` sign. Write code that uses the accumulation pattern to compute the **average** (sum divided by number of items) and assigns it to ``avg_temp``. Do not hard code your answer. If you do not know what the term "hard coding" means, have a look at section 2.14.  (You should use the ``.split(",")`` method on ``week_temps`` to split by ``","`` and ``float()`` to cast to a float).
